...
def Calculate(expr):
    ...
    # Procesar fracciones
    while ':' in Tokens:
        for index, func in enumerate(Tokens):
            if func == ':':
...
def Button_Erase():
    ...
    sequences = {"Ans": len("Ans"), "sin": len("sin"), "cos": len("cos"), "tan": len("tan"), "√": len("√")}
...
def Button_Craft(parent, text, command, row, col, rowspan=1, colspan=1, bg="#131313", fg="white"):
    button = Button(parent, text=text, borderwidth=0, highlightthickness=0, relief="flat", command=command,
                    padx=10, pady=10, width=6, height=1, font=('Segoe UI', 20), bg=bg, fg=fg)
...
Button_Craft(mainframe, "^", lambda: button_click("^"), 3, 1)
